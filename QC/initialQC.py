...
if __name__ == '__main__':
    ...
    df_out = pd.DataFrame(columns = ['sample_id', 'sample_date', 'gor', 'shrinkage', 'location', 'applied_average','lower_limit','upper_limit','status'])
    for df in df_list:
    ...
    for index, row in df_out.iterrows():
        ...
        update_query = "UPDATE shrinkage SET location=?, applied_average=?, lower_limit=?, upper_limit=?, status=? WHERE sample_id=?"
        cursor.execute(update_query,(row.location, row.applied_average, row.lower_limit, row.upper_limit, row.status, row.sample_id,))
